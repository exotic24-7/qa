...
function processSpriteImage(key, img, opts){
    try{
        ...
        try{
            // Only perform color-key background stripping when caller
            // explicitly allows it. Use opts.noColorKey to preserve
            // the PNG's native alpha exactly (useful for bees/artwork).
            if(!(opts && opts.noColorKey)){
                // Determine one or more background colors to strip.
                // Support either a single RGB array ([r,g,b]) or an array of RGB arrays ([[r,g,b],...]).
                let bgColors = [];
                if(opts && opts.forceBgColor){
                    if(Array.isArray(opts.forceBgColor) && opts.forceBgColor.length && Array.isArray(opts.forceBgColor[0])){
                        bgColors = opts.forceBgColor;
                    } else if(Array.isArray(opts.forceBgColor) && opts.forceBgColor.length >= 3){
                        bgColors = [opts.forceBgColor.slice(0,3)];
                    }
                }
                if(bgColors.length === 0){
                    const t = octx.getImageData(0,0,1,1).data;
                    bgColors = [[t[0], t[1], t[2]]];
                }
                ...
                for(let i=0;i<data.data.length;i+=4){
                    ...
                    for(const bc of bgColors){
                        if(Math.abs(r - (bc[0]||0)) < tol && Math.abs(g - (bc[1]||0)) < tol && Math.abs(b - (bc[2]||0)) < tol){ match = true; break; }
                    }
                    if(match) data.data[i+3] = 0;
                }
                octx.putImageData(data,0,0);
            }
        }catch(e){}
        window.MOB_SPRITES[key] = { img, canvas: oc, w: w, h: h, loaded: true };
        // If we just loaded the bee sprite, update any existing bee mobs so
        // their hitbox radius matches the sprite's displayed size.
        if(key === 'bee'){
            try{
                const spr = window.MOB_SPRITES && window.MOB_SPRITES.bee;
                // Set existing bee mob radii so their base half-size equals
                // half the sprite max-dimension. The drawn size will then be
                // enlarged by `BEE_SCALE` in `drawBee`.
                const desiredRadius = Math.max(spr.w || 0, spr.h || 0) / 2;
                mobs = mobs || [];
                for(let i=0;i<mobs.length;i++){
                    const m = mobs[i];
                    const t = (m.type || m.name || '').toString().toLowerCase();
                    if(t === 'bee'){
                        m.radius = desiredRadius;
                    }
                }
            }catch(e){}
        }
...
// ====================
// Bee Sprite Setup (use MOB_SPRITES, preserve PNG alpha)
// ====================

// Load bee sprite and strip white + light-grey backgrounds if present.
// Pass multiple colors via forceBgColor: [[r,g,b], ...]
try{ loadMobSprite('bee', 'assets/bee.png', { forceBgColor: [[255,255,255],[220,220,220]], tolerance: 28 }); }catch(e){}

// Make bee face right by default: zero rotation corresponds to facing right.
// Use a small debug helper to cycle common offsets: 0, +90°, -90°, 180°.
let BEE_ROTATION_OPTIONS = [0, Math.PI/2, -Math.PI/2, Math.PI];
let BEE_ROT_INDEX = 0;
let BEE_ROT_OFFSET = BEE_ROTATION_OPTIONS[BEE_ROT_INDEX];
// Visual scale multiplier for bee sprite rendering (tweak to taste)
const BEE_SCALE = 1.6;

function cycleBeeRotation(dir = 1){
    BEE_ROT_INDEX = (BEE_ROT_INDEX + dir + BEE_ROTATION_OPTIONS.length) % BEE_ROTATION_OPTIONS.length;
    BEE_ROT_OFFSET = BEE_ROTATION_OPTIONS[BEE_ROT_INDEX];
    console.log('Bee rotation offset set to', BEE_ROT_OFFSET, ' (index', BEE_ROT_INDEX, ')');
    // expose to window for quick inspection
    window.BEE_ROT_OFFSET = BEE_ROT_OFFSET;
}

// Press 'B' to cycle bee rotation in the running game (helpful for tuning sprite)
document.addEventListener('keydown', function(e){ try{ if(e.key === 'b' || e.key === 'B'){ cycleBeeRotation(1); } }catch(ex){} });
...
function drawBee(ctx, x, y, radius = 16, rotation = 0) {
    ...
    // adjust facing
    ctx.rotate(rotation + BEE_ROT_OFFSET);

    ctx.imageSmoothingEnabled = false;

    // Preserve source aspect ratio and make the sprite's max dimension equal
    // to `radius * 2 * BEE_SCALE` so `radius` behaves like a base half-size.
    const maxSize = radius * 2 * (typeof BEE_SCALE === 'number' ? BEE_SCALE : 1);
    const aspect = (spr.w && spr.h) ? (spr.w / spr.h) : 1;
    let drawW = maxSize;
    let drawH = maxSize;
    if(aspect > 1){
        drawW = maxSize;
...
// keep lowercase alias for compatibility with existing calls
function drawbee(ctx, x, y, radius = 16, rotation = 0){ return drawBee(ctx, x, y, radius, rotation); }
        ...
        try{
            ...
            if(s.tpl){
                ...
                if(choice<0.25) mobs.push({x:s.x,y:s.y,radius:Math.round(12 * (1 + rIdx*0.06)),speed:Math.max(0.2,1.5/(Math.max(1,mult*0.8))),health:Math.round(50*mult),maxHealth:Math.round(50*mult),name:"Ladybug",type:"Ladybug",projectiles:[],rarityIndex:rIdx,rarityName:rName,stationary:false, mass: Math.round(12 * (1 + rIdx*0.06)), vx:0, vy:0});
                    ...
                    // default fallback radius used before sprite info available
                    const defaultRadius = Math.round(10 * (1 + rIdx*0.06));
                    const mobObj = {x:s.x,y:s.y,radius: defaultRadius, speed:Math.max(0.2,2/(Math.max(1,mult*0.8))),health:Math.round(30*mult),maxHealth:Math.round(30*mult),name:"Bee",type:"Bee",projectiles:[],rarityIndex:rIdx,rarityName:rName,stationary:false, mass: Math.round(10 * (1 + rIdx*0.06)), vx:0, vy:0};
                    try{
                        const spr = window.MOB_SPRITES && window.MOB_SPRITES.bee;
                        if(spr && spr.loaded){
                            // Set mob radius to half the sprite's max dimension; `drawBee`
                            // will apply `BEE_SCALE` to the final drawn size.
                            mobObj.radius = Math.max(spr.w || 0, spr.h || 0) / 2;
                        }
                    }catch(e){}
                    mobs.push(mobObj);
                }
...
// Track rotation for each mob type
const mobRotations = {};

// Listen for M key to toggle menu
document.addEventListener('keydown', e => {
    try{
        if(typeof e.key === 'string' && e.key.toLowerCase() === 'm'){
            showMobMenu = !showMobMenu;
            mobMenuDiv.style.display = showMobMenu ? 'block' : 'none';
            if(showMobMenu) updateMobMenu();
        }
    }catch(ex){}
});

// Create the menu container
const mobMenuDiv = document.createElement('div');
mobMenuDiv.style.position = 'fixed';
mobMenuDiv.style.top = '50px';
mobMenuDiv.style.right = '20px';
mobMenuDiv.style.background = 'rgba(0,0,0,0.8)';
mobMenuDiv.style.color = 'white';
mobMenuDiv.style.padding = '10px';
mobMenuDiv.style.fontFamily = 'sans-serif';
mobMenuDiv.style.maxHeight = '80vh';
mobMenuDiv.style.overflowY = 'auto';
mobMenuDiv.style.display = 'none';
mobMenuDiv.style.zIndex = 9999;
mobMenuDiv.style.borderRadius = '8px';
mobMenuDiv.style.minWidth = '260px';
document.body.appendChild(mobMenuDiv);

// Helper to draw a rotated image onto a small canvas
function drawRotatedToCanvas(srcCanvasOrImg, rotRad, size=48){
    ...
    cx.rotate(rotRad || 0);
    const s = size;
    try{ cx.drawImage(srcCanvasOrImg, -s/2, -s/2, s, s); }catch(e){}
    cx.restore();
    return c;
}
...
function updateMobMenu() {
    try{
        mobMenuDiv.innerHTML = '<h3 style="margin:6px 0 10px 0">Mobs Debug</h3>';

        const keys = new Set(Object.keys(window.MOB_SPRITES || {}));
        Array.from(keys).sort().forEach(key => {
            const mob = window.MOB_SPRITES[key];
            if(!mob || !mob.loaded) return;

            if(mobRotations[key] === undefined) mobRotations[key] = 0;
            ...
            const inGameCanvas = document.createElement('canvas');
            inGameCanvas.width = 64; inGameCanvas.height = 64;
            const icx = inGameCanvas.getContext('2d');
            icx.clearRect(0,0,64,64);
            if(mob.canvas || mob.img){
                try{
                    const drawn = drawRotatedToCanvas(mob.canvas || mob.img, mobRotations[key], 48);
                    icx.drawImage(drawn, 0, 0);
                }catch(e){}
            }
            container.appendChild(inGameCanvas);

            const separateCanvas = document.createElement('canvas');
            separateCanvas.width = 64; separateCanvas.height = 64;
            const scx = separateCanvas.getContext('2d'); scx.clearRect(0,0,64,64);
            if(mob.img){
                try{
                    const imgCanvas = drawRotatedToCanvas(mob.img, mobRotations[key], 40);
                    scx.drawImage(imgCanvas, 0, 0);
                }catch(e){}
            } else {
                try{ const imgCanvas = drawRotatedToCanvas(mob.canvas || mob, mobRotations[key], 40); scx.drawImage(imgCanvas,0,0); }catch(e){}
            }
            container.appendChild(separateCanvas);
            ...
            slider.value = (mobRotations[key] || 0) * (180/Math.PI);
            slider.style.flex = '1';
            slider.addEventListener('input', e=>{
                const deg = parseFloat(e.target.value || 0);
                mobRotations[key] = deg * Math.PI/180;
                console.log(key + ' rotation (radians):', mobRotations[key]);
                updateMobMenu();
            });

            const num = document.createElement('input');
            num.type = 'number'; num.min = 0; num.max = 360; num.value = slider.value;
            ...
            mobMenuDiv.appendChild(container);
        });
    }catch(e){ console.warn('updateMobMenu failed', e); }
}

// Simple RAF loop to keep menu responsive without interfering with game loop
function mobMenuRenderLoop(){
    try{
        mobMenuDiv.style.display = showMobMenu ? 'block' : 'none';
        if(showMobMenu) updateMobMenu();
    }catch(e){}
    requestAnimationFrame(mobMenuRenderLoop);
}

if(!window._mobMenuLoopStarted){ window._mobMenuLoopStarted = true; mobMenuRenderLoop(); }
